...
class BaseGraphModule(pl.LightningModule, ABC):
    ...
    def __init__(
        ...
        *,
        ...
        loss_kwargs = {
            "nlat": torch.unique(graph_data[config.graph.data].x[:, 0]).shape[0],
            "nlon": torch.unique(graph_data[config.graph.data].x[:, 1]).shape[0],
            "nnodes": graph_data[config.graph.data].x.shape[0],
        }
